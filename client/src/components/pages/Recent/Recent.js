...
function Recent() {
	...
	function fetchPairings() {
		...
		fetch('/api/mongodb/quarantine-chill/test').then((response) => response.json()).then((data) => {
	...
	if (pairings === null) {
		return (
			<div className='recent-loader'>
				<FadeLoader size={2500} color={'#000000'} />
			</div>
		);
	} else {
		return (
				...
				<div className='recent-page'>
					<div style={{ marginTop: '120px' }}>
						...
						<div className='page-nav'>
							<div className='page-change-prev' onClick={goToPreviousPage}>
								⬅ Previous Page{' '}
							</div>
							<div className='page-change-next' onClick={goToNextPage}>
								Next Page ➡
							</div>
						</div>
						<div className='current-page'>
							{' '}
							Page {page} of {Math.ceil(pairings.length / 4)}{' '}
						</div>
					</div>
				</div>
			</div>
